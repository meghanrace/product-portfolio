...
`/resources/` uses a sticky guide index (`.lib` grid: `.lib-rail` + `.lib-body`), the same idea as the sidebar inside the two guides. Each guide card carries an id (`claude-101`, `claude-playbook`, `claude-code-pm`, `llm-context`, `cowork`) that the rail anchors to, with IntersectionObserver scroll-spy. The rail is hidden below 1000px. Add a guide by adding a card *and* its rail entry, or the two fall out of sync.

**Claude 101 and The Claude Playbook are different from every other page on this site.** They are authored in `projects/claude-guides/` and ported here. They do not load `styles.css`: they carry their own complete design system (Fraunces + IBM Plex, warm paper palette, sidebar TOC with scroll-spy) because their `.shell` and `.brand` rules collide with the site's. The site nav and footer on those pages are generated by the port script from the site's real nav rules, using `--s-*` prefixed tokens so they cannot clobber the guides' own `--bg` / `--ink`. The guides' own `.brand` is renamed `.docbrand` during the port. They have no dark mode: the port strips the theme toggle to match the rest of the site.
